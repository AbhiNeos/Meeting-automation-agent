...
def process_meeting_transcript(session: dict) -> dict:
    """
    Orchestrates the process of creating an MOM and then creating Jira tickets
    if action items are found.
    """ 
    mom_data = session["moms"][-1]["mom"]
    if not mom_data or "action_items" not in mom_data:
        return {"status": "success", "message": "MOM created, but no action items were found to process."}
    ...
    for item in mom_data.get("action_items", []):
        ...
        if re.search(r'\b(Jira|JIRA|ticket|create ticket)\b', action_text, re.IGNORECASE):
            ...
            project_key = "KAN" # <<-- UPDATE THIS WITH YOUR KEY

            tools_logger.info(f"Detected Jira action. Calling create_jira_ticket for: {action_text}")
            jira_result = create_jira_ticket(session, summary, description, project_key)
            jira_results.append(jira_result)

    final_message = "Meeting transcript processed. MOM created."
    if jira_results:
        successful_tickets = [r for r in jira_results if r["status"] == "success"]
        failed_tickets = [r for r in jira_results if r["status"] == "error"]
        
        if successful_tickets:
            ticket_ids = ", ".join([r["ticket_id"] for r in successful_tickets])
            final_message += f" Successfully created Jira tickets: {ticket_ids}."
        if failed_tickets:
            final_message += f" Failed to create {len(failed_tickets)} Jira tickets."
    else:
        final_message += " No Jira tickets were created."
...
root_agent = Agent(
    ...
    tools=[fetch_and_process_url, send_email],
)
